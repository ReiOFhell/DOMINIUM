...
class ProfileBackupController extends StateNotifier<GlobalBackupState> {
  ...
  static const Duration _cloudSyncTimeout = Duration(seconds: 25);
  ...
  Future<void> _processInternal({void Function(String domain)? onDomain}) async {
    while (true) {
      ...
      try {
        ...
        await _treasurySyncService.sync().timeout(
          _cloudSyncTimeout,
          onTimeout: () => throw const GlobalBackupTimeoutException(
            'Tempo limite excedido no backup em nuvem (treasury_entries).',
          ),
        );
...
class GlobalBackupTimeoutException implements Exception {
  const GlobalBackupTimeoutException(this.message);
